...
def bottles_song(beer):
    ...
    if beer >= 2: #Do this if the value of beer is bigger or equal to 2
        ...
        if beer == 1: # checks to see if beer changed to 1 and uses verse 1 without the s
            ...
            bottles_song(beer) #calls itself to run one final time.  
        else:    
        ...
        sys.exit #exits because I am sure I just created a monster with bottles_song calling itself.

beer = int(input(' How many beers do you have? and I will sing for each one! \n'))
if beer == type(int):
    if beer < 0:
        print(" you can't have negative beers!")
        sys.exit
else:
    bottles_song(beer)